...
// Retourne true si l'email a bien été transmis, false sinon (hors-ligne, service indisponible…).
...
async function handleSubmit(e) {
  ...
  const sent = await sendEmailAlert('Nouvelle demande de devis — MNGL Clean', {
    'Nom': nom,
    'Téléphone': tel,
    'Email': email,
...
// qui a réservé.
const GOOGLE_SCRIPT_URL = ''; // ex: 'https://script.google.com/macros/s/AKfycb.../exec'
...
// Retourne true si l'envoi a réussi, false sinon (silencieux : ne bloque jamais
// la réservation elle-même, qui reste valable même sans cet email).
async function sendClientConfirmationEmail(record, dateLabelLong, heureLabel) {
  if (!hasClientEmailBackend() || !record.email) return false;
  try {
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
      to_email: record.email,
      to_name: record.nom || 'Client',
...
(function () {
  ...
  // Exemple avec 60 min de marge : un "Complet" (2h30) démarré à 10h bloque
  // jusqu'à 13h30 (10h + 2h30 + 1h) — 13h30 et après restent libres.
  const MARGE_APRES_RDV_MIN = 60; // ← passe à 30 si tu préfères une marge plus courte
  ...
  async function renderSlots() {
    ...
    slotGrid.innerHTML = '<p class="booking-note">Vérification des disponibilités…</p>';
    ...
    for (let start = OPEN_MIN; start + state.duration <= CLOSE_MIN; start += SLOT_STEP) {
  ...
  function renderRecap() {
    ...
    recapBox.innerHTML = `
      <span><strong>${state.service}</strong> — à partir de ${state.price}€</span>
      <span>${dateLabel} à ${state.slotLabel}</span>
    `;
  }
  ...
  async function confirmBooking() {
    ...
    const record = {
      ...
      price: state.price,
      nom, tel, email, vehicule, adresse,
    ...
    const [sent] = await Promise.all([
      sendEmailAlert(`Nouvelle réservation — ${state.service} le ${dateLabelLong}`, {
        ...
        'Formule': `${state.service} (à partir de ${state.price}€)`,
    ...
    if (sent) {
      confirmText.textContent = hasClientEmailBackend()
        ? `${nom || 'Votre rendez-vous'} — formule ${state.service}, le ${dateLabelLong} à ${state.slotLabel}. Un email de confirmation vient de vous être envoyé (avec un lien pour annuler si besoin), et notre équipe a été alertée.`
        : `${nom || 'Votre rendez-vous'} — formule ${state.service}, le ${dateLabelLong} à ${state.slotLabel}. Notre équipe a été alertée automatiquement par email et vous recontacte rapidement pour confirmer l'intervention.`;
    } else {
      // Repli : ouvre le client mail du visiteur si l'alerte automatique échoue (ex: hors-ligne)
      confirmText.textContent = `${nom || 'Votre rendez-vous'} — formule ${state.service}, le ${dateLabelLong} à ${state.slotLabel}. L'alerte automatique n'a pas pu être envoyée (connexion indisponible) : un email de confirmation va s'ouvrir dans votre messagerie, merci de l'envoyer, ou appelez-nous directement au +33 6 71 67 97 28.`;
...
mnglclean.fr`
  ...
  confirmIcsBtn.addEventListener('click', () => {
    ...
    const ics = [
      ...
      `DESCRIPTION:Rendez-vous detailing automobile (${b.service}) avec MNGL Clean. Vehicule : ${b.vehicule || 'non précisé'}.`,
      `LOCATION:${b.adresse || 'À domicile — Vaucluse & Alentours'}`,
      'END:VEVENT',
      'END:VCALENDAR'
    ].join('\r\n');
