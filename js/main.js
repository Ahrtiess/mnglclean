...
async function handleSubmit(e) {
  ...
  // Enregistre aussi la demande dans le Google Sheet (feuille "Devis" +
  // mise à jour de la fiche client), si le calendrier partagé est
  // configuré. Ne bloque jamais l'envoi de l'email ci-dessus si ça échoue.
  if (typeof GOOGLE_SCRIPT_URL === 'string' && GOOGLE_SCRIPT_URL.startsWith('https://script.google.com/')) {
    fetch(GOOGLE_SCRIPT_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'text/plain;charset=utf-8' },
      body: JSON.stringify({ action: 'devis', nom, tel, email, prestation, vehicule, message })
    }).catch(() => { /* silencieux : l'email reste le canal principal */ });
  }
...
(function () {
  ...
  window.openBooking = function (service) {
    ...
    if (service) {
      ...
      goToStep(1); // reste sur l'étape 1 : il faut choisir le type de véhicule avant de continuer
    } else {
      goToStep(1);
    }
  };
  ...
  function resetState() {
    ...
    if (bookingExtras) bookingExtras.hidden = true;
    if (bookingOptionsField) bookingOptionsField.hidden = true;
  ...
  function selectService(card) {
    ...
    // Le type de véhicule se choisit pour toutes les formules ; les options
    // supplémentaires (traitements…) uniquement pour Intérieur et Complet.
    if (bookingExtras) bookingExtras.hidden = false;
    const needsOptions = state.service === 'Intérieur' || state.service === 'Complet';
    if (bookingOptionsField) bookingOptionsField.hidden = !needsOptions;

    // Sur mobile, les 3 formules remplissent tout l'écran : on fait défiler
    // automatiquement vers le choix du véhicule pour que ce soit évident
    // qu'il y a une suite, sans que le client ait à deviner.
    if (bookingExtras) {
      requestAnimationFrame(() => {
        bookingExtras.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    }
