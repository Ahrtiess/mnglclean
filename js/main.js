...
(function () {
  ...
  function isStepValid(n) {
    if (n === 1) {
      ...
      if (!NO_VEHICLE_TYPE_SERVICES.includes(state.service) && !state.vehicleType) return false;
      return true;
    }
  ...
  function selectService(card) {
    ...
    // Le type de véhicule n'a pas d'impact sur certaines prestations (ex. phares) : on le masque
    if (vehicleTypeField) {
      vehicleTypeField.hidden = NO_VEHICLE_TYPE_SERVICES.includes(state.service);
    }
