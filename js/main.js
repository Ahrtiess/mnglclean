...
async function handleSubmit(e) {
  ...
  // Anti-spam : si ce champ caché est rempli, c'est un robot — on bloque
  // silencieusement sans rien envoyer, le formulaire semble fonctionner normalement.
  if (form.site_web && form.site_web.value) return;
...
(function () {
  ...
  let state = {
    step: 1,
    service: null, services: [], price: null, duration: null,
  ...
  function resetState() {
    state = {
      step: 1,
      service: null, services: [], basePrice: null, price: null, baseDuration: null, duration: null,
    ...
    if (vehicleTypeField) vehicleTypeField.hidden = false;
  ...
  // Formules pour lesquelles le type de véhicule n'a pas d'impact sur le prix
  const NO_VEHICLE_TYPE_SERVICES = ['Rénovation Phares'];

  function isStepValid(n) {
    if (n === 1) {
      if (!state.services || state.services.length === 0) return false;
      const needsVehicle = state.services.some(s => !NO_VEHICLE_TYPE_SERVICES.includes(s.name));
      if (needsVehicle && !state.vehicleType) return false;
      return true;
    }
  ...
  const vehicleTypeField   = document.getElementById('vehicle-type-field');

  // Ces 4 formules représentent chacune un niveau d'intérieur (seul ou combiné à
  // l'extérieur) : elles se recouvrent, donc on n'en choisit qu'une à la fois.
  const EXCLUSIVE_GROUP = ['Basique', 'Premium', 'Standard', 'Complet'];

  // Standard et Complet incluent déjà l'extérieur : impossible de cumuler avec Extérieur seul.
  const INCLUDES_EXTERIEUR = ['Standard', 'Complet'];

  function deselectCard(otherName) {
    const otherCard = overlay.querySelector(`.bsc[data-service="${otherName}"]`);
    if (otherCard && otherCard.classList.contains('is-selected')) {
      otherCard.classList.remove('is-selected');
      state.services = state.services.filter(s => s.name !== otherName);
    }
  }

  overlay.querySelectorAll('.bsc').forEach(card => {
    card.addEventListener('click', () => { toggleService(card); updateNavUI(); });
  });
  ...
  function toggleService(card) {
    ...
    if (willSelect) {
      // Extérieur seul n'a pas de sens si Standard/Complet (qui l'incluent déjà) est actif.
      if (name === 'Extérieur' && INCLUDES_EXTERIEUR.some(n => overlay.querySelector(`.bsc[data-service="${n}"]`)?.classList.contains('is-selected'))) {
        return;
      }

      // Si on sélectionne une formule du groupe exclusif, on désélectionne
      // d'abord toute autre formule du même groupe déjà choisie.
      if (EXCLUSIVE_GROUP.includes(name)) {
        EXCLUSIVE_GROUP.filter(n => n !== name).forEach(deselectCard);
      }

      // Standard/Complet incluent déjà l'extérieur : on retire Extérieur s'il était actif.
      if (INCLUDES_EXTERIEUR.includes(name)) {
        deselectCard('Extérieur');
      }
    }
    ...
    // Résumé lisible pour le récap / l'email / le calendrier
    state.service      = state.services.map(s => s.name).join(' + ');
    state.basePrice    = state.services.reduce((sum, s) => sum + s.price, 0);
    state.baseDuration = state.services.reduce((sum, s) => sum + s.duration, 0);
    state.duration      = state.baseDuration;

    // Le type de véhicule n'a pas d'impact si SEULES des prestations qui n'en
    // ont pas besoin (ex. phares) sont sélectionnées. Dès qu'une autre
    // prestation est ajoutée, il redevient nécessaire.
    const needsVehicle = state.services.some(s => !NO_VEHICLE_TYPE_SERVICES.includes(s.name));
    if (vehicleTypeField) vehicleTypeField.hidden = state.services.length > 0 && !needsVehicle;
  ...
  function renderRecap() {
    ...
    const rows = [
      ...
      state.vehicleType ? { label: 'Type de véhicule', value: state.vehicleType } : null,
  ...
  async function confirmBooking() {
    // Anti-spam : si ce champ caché est rempli, c'est un robot — on bloque silencieusement.
    if (form.site_web && form.site_web.value) return;
...
(function () {
  ...
  const devisTitle = document.getElementById('devis-title');
  const devisIntro = document.getElementById('devis-intro');
  const devisFieldLabel = document.getElementById('devis-field-label');
  const devisFieldInput = document.getElementById('devis-field-input');
  const devisPrestation  = document.getElementById('devis-prestation');
  const devisGroupAuto    = document.getElementById('devis-group-auto');
  const devisGroupTextile = document.getElementById('devis-group-textile');

  window.openDevis = function (context, presetPrestation) {
    if (devisForm) devisForm.reset();

    if (context === 'textile') {
      devisTitle.textContent = 'Nettoyage textile';
      devisIntro.textContent = "Matelas, canapé, tapis, chaise… décris-nous l'article et son état, on te fait un devis rapidement.";
      devisFieldLabel.textContent = "Type d'article";
      devisFieldInput.placeholder = 'Ex : canapé 3 places, matelas 140x190…';
      if (devisGroupAuto) devisGroupAuto.hidden = true;
      if (devisGroupTextile) devisGroupTextile.hidden = false;
    } else {
      devisTitle.textContent = 'Un besoin particulier ?';
      devisIntro.textContent = 'Véhicule atypique, prestation sur-mesure ou question avant de réserver : décris-nous ta demande, on te répond rapidement.';
      devisFieldLabel.textContent = 'Véhicule';
      devisFieldInput.placeholder = 'Marque, modèle, année';
      if (devisGroupAuto) devisGroupAuto.hidden = false;
      if (devisGroupTextile) devisGroupTextile.hidden = true;
    }
    if (devisPrestation) devisPrestation.value = presetPrestation || '';
...
// ══════════════════════════════════════════════
// ── BANDEAU COOKIES : ne s'affiche qu'une fois ──
// ══════════════════════════════════════════════
(function () {
  const banner = document.getElementById('cookie-banner');
  if (!banner) return;

  const CONSENT_KEY = 'mngl_cookie_consent';
  const existing = localStorage.getItem(CONSENT_KEY);

  // Si la personne a déjà répondu (accepté ou refusé) lors d'une visite précédente,
  // on ne réaffiche jamais le bandeau, même après rafraîchissement.
  if (!existing) {
    banner.hidden = false;
  }
  ...
  document.getElementById('cookie-accept').addEventListener('click', () => {
    ...
    banner.hidden = true;
    if (typeof mnglLoadAnalytics === 'function') mnglLoadAnalytics();
  });
