...
(function () {
  ...
  function resetState() {
    state = {
      step: 1,
      service: null, basePrice: null, price: null, duration: null,
  ...
  function recalcPrice() {
    ...
    if (priceTotalEl) {
      priceTotalEl.textContent = state.basePrice ? `Total estimé : ${state.price}€` : '';
    }
  }
  ...
  if (bookingOptionsWrap) {
    bookingOptionsWrap.querySelectorAll('.opt-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (isSelected) {
          state.options[key] = { supplement: Number(btn.dataset.supplement) || 0 };
        } else {
