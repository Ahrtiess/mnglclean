...
const HEADERS = ['id', 'date', 'start', 'duration', 'service', 'price', 'prenom', 'nom', 'tel', 'email', 'vehicule', 'adresse', 'status', 'createdAt'];
  ...
  if (body.action === 'cancel') {
    ...
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][0]) === String(body.id)) {
        ...
        sheet.getRange(i + 1, 13).setValue('annule'); // colonne "status" = 13e colonne
